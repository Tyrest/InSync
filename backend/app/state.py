...
def hydrate_jellyfin_from_db(db: Session) -> None:
    ...
    if jellyfin_url and jellyfin_api_key:
        app_state.jellyfin.base_url = jellyfin_url.value.rstrip("/")
        app_state.jellyfin.api_key = jellyfin_api_key.value
...
def hydrate_audio_config_from_db(db: Session) -> None:
    ...
    if fmt:
        app_state.downloader.audio_config.format = fmt
    if quality:
        app_state.downloader.audio_config.quality = quality
...
def seed_jellyfin_from_env_if_db_incomplete(db: Session) -> None:
    ...
    set_setting(db, "jellyfin_url", url_value)
    set_setting(db, "jellyfin_api_key", key_value)
    db.commit()
...
def bootstrap_jwt_secret(db: Session) -> str:
    ...
    set_setting(db, "jwt_secret", effective_secret)
    db.commit()
